...
if (!(Test-Path -Path $ShortcutPath)) {
    New-Shortcut -Path $ShortcutPath -TargetPath $MonitorVlc -Description "Monitor VLC to manage screensaver" -WindowStyle 7 -Icon "$Env:SystemRoot\system32\shell32.dll, 25" -WorkDir $(Split-Path $MonitorVlc)
}
